...
- Name: landing.spec.ts >> Landing page >> primary CTA links to signup
...
Locator: getByRole('link', { name: /create workspace/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('link', { name: /create workspace/i }).first()
...
```ts
  ...
  24 |     const cta = page.getByRole('link', { name: /create workspace/i }).first();
