		...
		async function* iterator() {
			...
			try {
				for await (const sse of _iterSSEMessages(response, controller)) {
					...
					if (sse.event === "message_start" || sse.event === "message_delta" || sse.event === "message_stop" || sse.event === "content_block_start" || sse.event === "content_block_delta" || sse.event === "content_block_stop" || sse.event === "message") try {
						yield JSON.parse(sse.data);
					} catch (e) {
						logger.error(`Could not parse message into JSON:`, sse.data);
						logger.error(`From chunk:`, sse.raw);
...
//#endregion
//#region ../../node_modules/@anthropic-ai/sdk/lib/tools/CompactionControl.mjs
...
var BaseAnthropic = class {
	...
	*/
	...
	getUserAgent() {
		return `${this.constructor.name}/JS 0.88.0`;
	}
