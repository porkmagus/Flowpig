...
minimatch.sep = defaultPlatform === "win32" ? path$2.win32.sep : path$2.posix.sep;
...
function getDevRouteMappings() {
	...
	try {
		const vercelJsonPath = path$1.join(process.cwd(), "vercel.json");
...
function findMappingsForFile(absolutePath) {
	...
	let relative2;
	try {
		relative2 = path$1.relative(cwd, absolutePath);
	} catch {
		return [];
	}
...
function getSdkPackageDir() {
	...
	try {
		const thisDir = typeof __dirname !== "undefined" ? __dirname : path$1.dirname(new URL(import.meta.url).pathname);
		_sdkPackageDir = path$1.resolve(thisDir, "..");
	} catch {
		_sdkPackageDir = "";
	}
...
function extractCallerFilePath() {
	...
	for (const line of lines) {
		...
		if (!fp) continue;
		const absolute = path$1.isAbsolute(fp) ? fp : path$1.resolve(process.cwd(), fp);
		let realFp;
		try {
...
function registerHandlerForFile(filePath, handler, client, options) {
	const fileMappings = findMappingsForFile(path$1.isAbsolute(filePath) ? filePath : path$1.resolve(process.cwd(), filePath));
...
function registerDevHandler(handler, client, options, _testCallerPath) {
	...
	if (!registerHandlerForFile(callerPath, handler, client, options)) {
		...
		let relative2;
		try {
			relative2 = path$1.relative(cwd, callerPath).replace(/\\/g, "/");
		} catch {
...
async function ensureHandlersLoaded(topicName, options = {}) {
	...
	for (const route of matchingRoutes) {
		...
		if (!alreadyRegistered) {
			const absolutePath = path$1.resolve(process.cwd(), route.filePath);
			try {
